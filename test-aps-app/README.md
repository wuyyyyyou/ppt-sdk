...
这个 Anna App 用于测试 Host Upload 的两段链路：

1. **后端上传**：Node Executa 插件调用 `host/uploadFile`，支持 `inline` 和 `negotiate + PUT + confirm`。
2. **前端下载**：App 前端拿后端返回的短期 `url`，先尝试 `fetch` 校验，再触发浏览器下载。
...
真实环境中，还需要 Anna Admin 为该 Executa 开启 `upload_grant`。初始化响应按官方 Host Upload 示例声明 `client_capabilities: { upload: {} }`。

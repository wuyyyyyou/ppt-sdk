...
PLUGIN_NAME="presenton-html-to-pptx-model-plugin"
...
if [[ "$RUN_TEST" == "true" ]]; then
  ...
  echo "$RESULT" | node -e "let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{const r=JSON.parse(d);process.exit(r.result&&r.result.name==='presenton-html-to-pptx-model-plugin'?0:1)})"

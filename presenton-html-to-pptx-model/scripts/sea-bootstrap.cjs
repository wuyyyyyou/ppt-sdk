...
const CACHE_ROOT = path.join(os.tmpdir(), "presenton-html-to-pptx-model-sea");
...
function ensureExtractedApp() {
  ...
  const entryPath = path.join(bundleRoot, manifest.entry);
  const readyMarker = path.join(bundleRoot, ".ready");

  if (!existsSync(readyMarker) || !existsSync(entryPath)) {

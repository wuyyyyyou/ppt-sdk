...
const OUTLINE_STORYLINE_RULES = [
  ...
  "Treat the confirmed slide_count as the total number of pages, including any cover and closing page.",
  "For a multi-page, audience-facing narrative deck such as an introduction, explainer, educational deck, report, proposal, training deck, or pitch, reserve the first page for a Formal Presentation Cover by default. The cover should establish the deck title, topic or context, audience or purpose, and only grounded identity or date metadata available from the Workspace.",
  "Omit a separate cover only when the deck is a one-page artifact, the Brief or confirmed requirements explicitly request no cover, or the material is clearly an internal continuous artifact such as a weekly report, dashboard, worksheet, appendix, or meeting follow-up that enters directly into an established context.",
  "When a cover is appropriate, do not use the first page for dense substantive content. Move the substantive storyline to later pages and compress or merge body content as needed to honor the confirmed total page count; a separate closing page is optional when the page count is small.",

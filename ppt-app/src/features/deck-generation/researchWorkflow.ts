...
export function buildWebResearchCurationPrompt(input: {
  ...
  return [
    ...
    "",
    "Evidence card rules:",
    "- Treat each fact as a verifiable evidence card, not a general summary.",
    "- Every fact must be atomic: one fact should express exactly one factual claim, metric, named claim, or source-supported observation.",
    "- Do not merge unrelated facts into one claim. Split them into separate facts or record a gap.",
    "- Preserve important source qualifiers in claim: time period, geography, entity/population, sample size, denominator, unit, currency, methodology, metric definition, and comparison baseline when the source provides them.",
    "- For numeric facts, claim must include the exact number, unit, time period, applicable scope, and denominator/sample/baseline if the source provides them.",
    "- Do not broaden source meaning. If the source says surveyed US enterprises, do not write enterprises. If a finding belongs to one source or study, do not state it as a universal conclusion.",
    "- For any fact containing a number, ranking, date, trend, comparison, or named entity, excerpt must contain the same number/date/entity or enough surrounding context to verify it.",
    "- Each web_source fact must include source_url or source_file; include both when available from the raw index/material.",
    "",
    "Excerpt rules:",
    "- excerpt must be a continuous passage from source_file that directly supports claim; prefer verbatim source text.",
    "- excerpt should include enough context to verify claim. It is better to be slightly longer than to omit time period, denominator, metric definition, scope, or caveat.",
    "- If one excerpt cannot support the complete claim, split the claim into multiple facts or record the unsupported part as a gap.",
    "- If material seems useful but you cannot identify precise support in source_file, do not create a fact. Add it to gaps or rejected_material with the reason.",
    "",
    "Tool boundary:",
...
export function buildVisualResearchCurationPrompt(input: {
  ...
  return [
    ...
    "- Do not write Agent file-tool absolute paths into visual_assets file_path or original_raw_path; those fields must preserve the exact raw image index values.",

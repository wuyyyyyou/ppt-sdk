...
function markRemoteSvgPending(delta: 1 | -1) {
  if (typeof window === "undefined") {
    return;
  }

  const current = window.__PRESENTON_REMOTE_SVG_PENDING__ ?? 0;
  const next = Math.max(0, current + delta);
  window.__PRESENTON_REMOTE_SVG_PENDING__ = next;
}

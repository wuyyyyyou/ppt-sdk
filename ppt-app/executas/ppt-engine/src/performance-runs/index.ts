...
function createReportHtml(run: PerformanceRunRecord, parsed: ParsedEvents, locale: "en" | "zh") {
  ...
  const uploadMetric = metric("host_upload", finished.filter((event) => event.operation_name === "host_upload"));
  const uploadSizes = uploadMetric?.events
    .map((event) => event.attributes?.size_bytes)
    .filter((value): value is number => typeof value === "number" && Number.isFinite(value) && value >= 0) ?? [];
  const uploadTotalBytes = uploadSizes.reduce((sum, value) => sum + value, 0);
  const uploadAverageBytes = uploadSizes.length > 0 ? uploadTotalBytes / uploadSizes.length : null;

  const labels = zh ? {
    title: "PPT 性能测试报告", subtitle: "面向测试人员的业务流程耗时与交互反馈摘要", overview: "运行摘要", total: "测试总时长", generation: "整套生成耗时", clicks: "按钮点击", slowestButton: "最慢按钮反馈", issues: "异常操作", integrity: "数据完整性", slowButtons: "耗时前 10 按钮", buttonNote: "按钮耗时表示浏览器收到点击后到下一帧反馈的 UI 延迟，不代表后台任务完成时间。", rank: "排名", button: "按钮名称", count: "次数", interaction: "平均交互", feedbackAverage: "平均反馈", feedbackP95: "反馈 P95", maximum: "最大", workflow: "PPT 创建流程", workflowNote: "业务阶段显示系统处理时间，不包含测试人员阅读、编辑和确认时的停留时间。", generationBreakdown: "生成过程分解", generationNote: "墙钟耗时合并了并行区间，表示实际等待；累计耗时表示全部调用工作量。父子阶段可能相互包含，不能直接相加。", preparation: "生成准备", research: "研究过程", pages: "逐页生成", completion: "生成收尾", pageBreakdown: "页面生成明细", page: "页面", authoring: "创作", render: "渲染", renderFix: "渲染修复", visualReview: "视觉检查", visualReviewFix: "视觉修复", uploads: "上传性能", uploadNote: "当前统计来自前端 Host Upload 完成事件，单次耗时覆盖协商、文件传输和确认；不包含后端上传、自动重试分解或各阶段独立耗时。", uploadCount: "上传次数", uploadData: "总上传数据量", uploadAverage: "平均上传耗时", uploadP95: "上传耗时 P95", uploadMaximum: "最大上传耗时", uploadIssues: "上传异常", successful: "成功", averageSize: "平均文件大小", stage: "阶段", wallClock: "墙钟耗时", totalWork: "累计耗时", average: "平均", errors: "异常", slowOperations: "最慢业务操作", diagnostics: "技术诊断数据", diagnosticsNote: "用于开发排查的底层 Tool、backend 和 roundtrip 指标。", issueDetails: "错误、中断与数据丢失", noData: "本次运行没有相应数据。", events: "个事件", started: "开始", ended: "结束",
  } : {
    title: "PPT Performance Report", subtitle: "Business workflow timing and interaction feedback for testers", overview: "Run summary", total: "Run duration", generation: "Deck generation", clicks: "Button clicks", slowestButton: "Slowest button feedback", issues: "Operation issues", integrity: "Data integrity", slowButtons: "Top 10 slowest buttons", buttonNote: "Button timing measures browser UI feedback from click receipt to the next frame. It is not backend task completion time.", rank: "Rank", button: "Button", count: "Count", interaction: "Avg interaction", feedbackAverage: "Avg feedback", feedbackP95: "Feedback P95", maximum: "Max", workflow: "PPT creation workflow", workflowNote: "Business stages show system processing time and exclude time spent by the tester reading, editing, or confirming.", generationBreakdown: "Generation breakdown", generationNote: "Wall-clock time merges overlapping intervals and represents actual waiting time. Cumulative time represents total work. Parent and child stages may overlap and must not be added together.", preparation: "Generation preparation", research: "Research", pages: "Page generation", completion: "Generation completion", pageBreakdown: "Page generation details", page: "Page", authoring: "Authoring", render: "Render", renderFix: "Render fixes", visualReview: "Visual review", visualReviewFix: "Visual fixes", uploads: "Upload performance", uploadNote: "Current metrics come from completed frontend Host Upload events. Each duration covers negotiation, file transfer, and confirmation; backend uploads, retry breakdowns, and per-phase timings are not included.", uploadCount: "Upload count", uploadData: "Total uploaded data", uploadAverage: "Average upload duration", uploadP95: "Upload duration P95", uploadMaximum: "Maximum upload duration", uploadIssues: "Upload issues", successful: "Successful", averageSize: "Average file size", stage: "Stage", wallClock: "Wall clock", totalWork: "Cumulative", average: "Average", errors: "Issues", slowOperations: "Slowest business operations", diagnostics: "Technical diagnostics", diagnosticsNote: "Low-level Tool, backend, and roundtrip metrics for engineering diagnosis.", issueDetails: "Errors, interruptions, and data loss", noData: "No corresponding data was recorded in this run.", events: "events", started: "Started", ended: "Ended",
  };
  const uploadSummaryItems = uploadMetric ? [
    [labels.uploadCount, String(uploadMetric.events.length)],
    [labels.uploadData, uploadSizes.length > 0 ? formatBytes(uploadTotalBytes) : "—"],
    [labels.uploadAverage, formatDuration(uploadMetric.average)],
    [labels.uploadP95, formatDuration(uploadMetric.p95)],
    [labels.uploadMaximum, formatDuration(uploadMetric.max)],
    [labels.uploadIssues, String(uploadMetric.errors + uploadMetric.interrupted)],
  ].map(([label, value]) => `<div class="summary-item"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`).join("") : "";
  const uploadDetails = uploadMetric
    ? `<div class="table-wrap"><table><thead><tr><th>${labels.stage}</th><th>${labels.count}</th><th>${labels.successful}</th><th>${labels.errors}</th><th>${labels.wallClock}</th><th>${labels.totalWork}</th><th>${labels.averageSize}</th></tr></thead><tbody><tr><td><strong>${escapeHtml(operationLabel("host_upload", zh))}</strong><code>host_upload</code></td><td>${uploadMetric.events.length}</td><td>${uploadMetric.events.length - uploadMetric.errors - uploadMetric.interrupted}</td><td>${uploadMetric.errors + uploadMetric.interrupted}</td><td>${formatDuration(uploadMetric.wall_clock)}</td><td>${formatDuration(uploadMetric.total)}</td><td>${uploadAverageBytes === null ? "—" : formatBytes(uploadAverageBytes)}</td></tr></tbody></table></div>`

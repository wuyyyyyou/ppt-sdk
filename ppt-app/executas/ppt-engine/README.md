...
- 在后端受管 Chrome 中将整体静态 Deck HTML 直接转换为 `.pptx`。
...
普通构建会先从仓库内 `third_party/dom-to-pptx/` 重建 browser bundle，再生成 ESM Node artifacts、browser runtime artifacts、类型声明、Authoring Kit 资源、forkable templates 资源和保留的 template preview 资源。vendored `dist/` 不提交；构建后的运行时复制到 `dist/vendor/dom-to-pptx/`。
...
- 打包脚本的 `--test` 会解压最终 archive，校验 binary distribution manifest、`lib/app/`、vendored dom-to-pptx bundle 和浏览器资源，并发启动多个 `describe` 进程，并通过真实 JSON-RPC 调用完成 HTML 与 PNG 冒烟。
...
- Deck HTML 不含预览 viewer；`#presentation-slides-wrapper` 直接包含纵向、无间隙排列的 1280×720 slide shell。
- Tailwind 使用随 ppt-engine 和 SEA 二进制发布的固定 `@tailwindcss/browser` 4.3.2，正式 HTML 不加载 Tailwind CDN 或携带 Tailwind runtime。
- 截图和 PPTX 导出从重新打开的同一静态 HTML 生成。
...
App 通过 `app_start_pptx_export` 启动作业，并轮询 `app_get_pptx_export_status`。引擎验证 Final Deck Render 记录的 `deck.html`，在受管 Chrome 中临时加载 vendored bundle，直接生成 `output/deck.pptx`。状态持久化在 `output/pptx-export.json`；单进程 FIFO 并发度为 1。缺失或非法 Deck HTML、不可读取的图片资源会使作业失败，不会自动重建 Deck 或回退旧 PPTX。
...
- 整体静态 Deck HTML 到 PPTX 的内部转换。
- 工作区 artifact 读写、PPTX 导出作业和任务状态机。

...
本目录存放 `red-finance-v3` 模板组的可复用视觉组件。组件只负责稳定的视觉单元和导出友好的结构；页面级布局、业务文案和数据默认值应优先放在 `slides/*.tsx` 与 `data/*.json` 中。
...
V3 的 Agent 先选 blueprint，再把内容填入 slot。组件选择必须从 slot 类型出发，不要只凭组件名拼接。

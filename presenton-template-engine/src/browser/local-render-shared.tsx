...
function resolveBrowserLayout(
  ...
  return localResolver?.(layoutId) ?? window.__PRESENTON_LOCAL_LAYOUTS__?.[layoutId];
}

function readSlideContext(): BrowserRenderContext {
...
function renderLayout(input: {
  ...
  const slideData = {
    ...input.context.slideData,
    _logo_url__: input.context.theme.logoUrl ?? null,
    __companyName__: input.context.theme.companyName ?? null,
  };

  return <LayoutComponent data={slideData} />;
}
...
export function renderPresentonSlideWithErrorBoundary(
  localResolver?: BrowserLayoutResolver,
) {
      ...
      root.render(
        <div className="w-full" data-speaker-note={context.speakerNote}>
          <div data-layout={context.layoutId} data-group={context.templateGroup} className="w-full">
            {renderLayout({ context, localResolver })}
          </div>
        </div>,
      );
    });
...
export function renderPresentonDeckWithErrorBoundary(
  localResolver?: BrowserLayoutResolver,
) {
  const container = document.getElementById("presentation-slides-wrapper");
  ...
  try {
    ...
    for (const [index, context] of contexts.entries()) {
      ...
      flushSync(() => {
        root.render(renderLayout({ context, localResolver }));
      });
    }

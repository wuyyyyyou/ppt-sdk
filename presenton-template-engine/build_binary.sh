...
if [[ "$RUN_TEST" == "true" ]]; then
  ...
  export EXPECTED_PLUGIN_NAME="$PLUGIN_NAME"
  echo "$RESULT" | node -e "let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{const r=JSON.parse(d);process.exit(r.result&&r.result.name===process.env.EXPECTED_PLUGIN_NAME?0:1)})"

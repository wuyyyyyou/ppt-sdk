...
const PROJECT_DIR = path.join(WORKSPACE_DIR, "presenton-html-to-pptx-model");
...
async function main() {
  const stdinFile = resolvePath(process.env.MODEL_STDIN_FILE, DEFAULT_STDIN_FILE);
  const stdoutFile = resolvePath(process.env.MODEL_STDOUT_FILE, DEFAULT_STDOUT_FILE);
  ...
  process.stdout.write(`model stdout written to ${stdoutFile}\n`);

...
When a confirmed `application/json` Host Upload reference cannot be read directly by the browser because the platform download URL does not allow the App origin through CORS, the Anna adapter may retry that one read through `app_resolve_host_upload_json_reference`. The Executa performs the same short-lived HTTPS download outside the browser CORS boundary, verifies the declared MIME type and byte size, and returns at most 512 KiB through the existing compatibility response path. This is a bounded development/runtime compatibility measure rather than a general artifact proxy: larger JSON results still fail explicitly, non-JSON artifacts never use it, and the preferred platform fix remains a CORS-enabled or same-origin Host Upload download URL.

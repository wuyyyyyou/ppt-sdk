...
- The Page and Deck Refinement migration removes Page Plan from every user-reachable creation, resume, refinement, rendering, and export path. Repository-wide deletion remains a separate cleanup while sealed research implementations and the legacy task-state-machine still reference the old model; that cleanup must not silently broaden the Refinement migration into redesigning those sealed capabilities.

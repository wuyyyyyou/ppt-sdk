...
Invalidation is owned atomically by the engine operations that change Style Guide inputs. Updating Presentation Requirements deletes the artifact when Visual Tone changes. Every Outline persistence operation—manual Draft save, Rewrite Request automatic save, and Confirmation—compares the currently persisted Outline with the incoming normalized content and deletes the artifact when the deck title, page set, or page intent changes, but not when otherwise unchanged entries are only reordered. This avoids retaining a previous Confirmed Outline snapshot after a Draft overwrites the current artifact. The frontend never persists the new dependency state and deletes the Style Guide in a second call.
